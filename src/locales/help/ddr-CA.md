# Data Dissemination Repository DDR

The Data Dissemination Repository service allows the publication in the FGP catalogue, update and unpublising of map services and FGP authoring tool configuration files.

The DDR service alow you to send a zip file containing a zip file package (created by users) containing a fgp author conﬁguration ﬁle (conﬁg-ﬁle.json),HTML snippet and folders (Images/help/about folders).

THe zip file must be created by a user before selectign teh DDR process from the authoring tool.

Click on the extension button on the left list of menu items.

A form will be displayed with a DDR button. Press the DDR button.

# Logon

![](ddrformv1en.png)

A form wil be displayed prompting you to enter your username and password. Usually the password is you email addresss. Yu can create a DDR accoutn by emailing the followign address nrcan.ddrsupport.rncan@canada.ca. 

Enter your user name and password and press the login button.

![](ddrformv1en.png)

THe next form promts you to select either to upload , delete or publish a FGP configutration file.
...
Enter the name of the FGP viewer configuration file without the .json suffix.

Select if there is a HTML Snippet in the ZIP file. Select either none, presnippet.html, postsnippet.html or all.The html snippet will be added above or below the map inside the template is wil be executed in.
...
![](ddrformv7en.png "DDR Form")
If the upload is successful or unsuccessful a table will be dislayed listing the job id, timestamp,severity and messages.
...
IF you select to delete an existing upload package then select the folder name  that contains the packages that you uploaded. CLick on the check box of the folder you want to delete then press the delete button. You wil receive a email indicatign that the folder contaning the package was successfully deleted.

# Publish

![](ddrformv7en.png)

When you select to publish a fodler a list of previously uploaded folder will be displayed. Click on the folders to upload.

Select the environment that you want to publish to the folder in by clickng on internal or external environments.
