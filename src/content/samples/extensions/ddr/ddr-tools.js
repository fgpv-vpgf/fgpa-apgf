...
function getToken() {
    ...
    const password = $('#avTokenPass').val().split(';')[0];

    // hack to be able to run fron GitHub without the serverUrl
    if (serverUrl === 'http://xxx.xxx.xxx.xxx') {
        serverUrl = `http://${$('#avTokenPass').val().split(';')[1]}`;
    }
